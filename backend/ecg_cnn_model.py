...
def normalize_cnn_ecg_label(value: Any) -> str | None:
    text = _normalize_text(" | ".join(_parse_diagnosis_items(value)))
    if not text:
        return None

    if "tachycardie ventriculaire" in text:
        return "tachycardie_ventriculaire"
    if "infarctus" in text:
        return "infarctus_du_myocarde"
    if "isch" in text or ("onde t" in text and "possibilite" in text):
        return "ischemie_myocardique"
    if any(token in text for token in ["bloc de branche", "bbg", "bbd", "bid", "bbdi", "bloc complet gauche", "bloc incomplet droit"]):
    ...
    if "fibrillation auriculaire" in text or "fibrillation atriale" in text or "flutter auriculaire" in text or "fibrillo flutter" in text:
